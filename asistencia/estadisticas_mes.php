...
$results = $mysqli->query("SELECT matricula, nombre_usuario, HOUR(`registro`) as hora, MINUTE(`registro`) as minuto from asistencias WHERE MONTH(`registro`) = $mes_actual AND YEAR(`registro`) = $year_actual and estatus = 1");
...
<?php
//Toma el la hora de registro de la entrada
$results = $mysqli->query("SELECT matricula, nombre_usuario, HOUR(`registro`) as hora, MINUTE(`registro`) as minuto from asistencias WHERE MONTH(`registro`) = $mes_actual AND YEAR(`registro`) = $year_actual and estatus = 2");
while($row = $results->fetch_array())   
    {
    $matricula = $row["matricula"];
    $nombre_usuario = $row["nombre_usuario"];
